...
const Home = () => {
  ...
  const getDataTestnet = async () => {
    const apiUrl = `${import.meta.env.VITE_APP_API_ENDPOINT_SCHEMA_INFO}schema/list_testnet`;
    const params = {};
  ...
  const mockItems = [
    ...
    },
    {
      ...
      text: "Item 2",
      imageSrc: logo9,
      schema_code: "Whale Gate Collection",
      countColl: "600",
    },
  ];
          ...
          <Box className="h-full w-4/6 flex flex-col pl-[50px] pt-[50px] relative ">
            <Flex wrap={"wrap"} height={"832px"} overflow={"auto"}>
                ...
                <div
                  onClick={() => {
                    if (listDraft.length !== 0) {
                      navigate("newintregation/5");
                    } else {
                      navigate("newintregation/1");
                    }
                  }}
                >
                  <HomeDraftCard
              ...
              <Flex wrap={"wrap"} paddingBottom={"50px"}>
                <Box>
                  <p className="text-[#FFFFFF] text-3xl mt-[30px] mb-1 ">
                    Live
                  </p>
                ...
                <Box overflow={"auto"} pt={60}>
                  <Flex
                    className="w-[782px] h-[323px] overflow-scroll "
                    pt={30}
                  >
                    {mockItems.map((item) => (
                        ...
                        >
                          ...
                          >
                            <div className="w-[197px] h-[69px] bg-[#D9D9D9]/[0.8] rounded-b-2xl absolute flex flex-row justify-center items-center">
                              ...
                              <p className="text-[#100072] text-[20px]">
                                {item.countColl} Items
                              </p>
                            </div>
                          </Box>
                        </Flex>
              ...
              <Flex wrap={"wrap"} paddingBottom={"50px"}>
                ...
                {loadingTestnet && listTestnet && (
                  <Box overflow={"auto"} pt={60}>
                    <Flex
                      className="w-[732px] h-[332px] overflow-scroll "
                      pt={30}
                    >
                      {listTestnet.map((item, index) => (
                        <Flex direction={"column"} alignItems={"center"}>
                          ...
                          <Text
                            pt={18}
                            className="text-white text-2xl font-bold mt-3  hover:text-[#7A8ED7] duration-500 "
                          >
                            {item.schema_name}
                          </Text>
                        </Flex>
                ...
                {!loadingTestnet && (
                  <Box overflow={"auto"} pt={60}>
                    <Flex
                      className="w-[782px] h-[300px] overflow-scroll "
                      pt={30}
                    >
                      {!loadingTestnet &&
