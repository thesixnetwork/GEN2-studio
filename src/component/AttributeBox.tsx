...
export default function AttributeBox(props: MyComponentProps) {
  ...
  const handleChange = (text: string, fieldPath: string) => {
...
//   };
  ...
  const CheckErrorI = async (e: React.ChangeEvent<HTMLInputElement>) => {
    setPartI(false);
    // if (!e.target.value) {
    //     setErrorMessage("Not Availible")
    //     setIser(true)
...
//   };
  ...
  const changeBgColorButton = async (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    // const itemId = e.target.id;
  ...
  const handleChangeIcon = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    const updatedText = [...props.text];
    // //(typeof e.target.id)
    if ((e.target as HTMLDivElement).id == "string") {
        ...
        {props.Value && props.Value !== undefined && props.Value !== null ? (
          <div className="flex text-[14px] items-center">
            ...
            <input
              type="text"
              value={props.Value}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange(e.target.value, "traitType")}
              className="bg-transparent text-[14px] border-[1px] border-[#D9D9D9DD] placeholder-gray-300 border-dashed p-1 focus:outline-none focus:scale-105 duration-1000 w-[140px]"
              placeholder="Add trait type here"
            />
          </div>
