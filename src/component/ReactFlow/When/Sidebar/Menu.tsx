...
const Menu = (prop: MenuProps) => {
  return (
    <div className="flex flex-col items-center w-14 transform translate-x-0 translate-y-0">
      ...
      <p className="text-xs">
        {prop.title === "V" ? "Value" : prop.title === "P" ? "Param" : prop.title === "@" ? "Attribute" : prop.title === ">" ? "GT" : prop.title === ">=" ? "GTE" : prop.title === "<" ? "LT" : prop.title === "<=" ? "LTE" : prop.title === "=" ? "Equal" : prop.title === "!=" ? "Not Equal" : prop.title === "OR" ? "Or" : prop.title === "AND" ? "And" :  null}
      </p>
    </div>
