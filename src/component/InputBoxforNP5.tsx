...
export default function InputBoxforNP5(props: MyComponentProps) {
  ...
  }, [props.text[props.index].value, props.Next]);
  ...
  return (
    <div
      id={`box${props.index}`}
      className="relative w-[658px] h-[121px] border-[1px] border-white rounded-xl p-2 flex items-center justify-center  "
    >
      <div className=" w-4/5 flex justify-start items-center ">
        <p className="font-bold text-[24px] w-2/5 mr-[5%]">
        ...
        <input
          ...
          className={` placeholder-slate-300 bg-transparent text-[24px] border-[1px] ${
            isDisabled ? "border-none" : " border-[#D9D9D9DD]"
          } border-dashed p-1 focus:outline-none focus:scale-105 duration-1000`}
        ></input>
        <div
          className={`w-[15px] h-[15px]  ${
            props.text[props.index]["require"] ? "bg-[#D9D9D9]" : "bg-transparent"
          } rounded-full absolute border top-2 right-2`}
        ></div>
      </div>
      {iser && (
