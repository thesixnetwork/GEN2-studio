        ...
        <Box sx={{ width: '100%' }} className='flex w-full  justify-between items-end mb-2'>
            <div className='flex w-full '>
                {steps.slice(0, props.CurrentState).map((label, index) => (
                    <Link to={index === 5 ? '/newintregation/beginer' : `/newintregation/${index + 5}`}
                        key={index}
                        className={`flex justify-center items-center ${index === props.ActiveStep - 1 ? '' : 'text-gray-400'}  cursor-pointer hover:scale-105 duration-500 ml-3`} >
